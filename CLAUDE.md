...
**「新宿→渋谷」のような複数エリアをまたぐ移動**: 現状は起点1ヶ所からの半径検索のみ対応（上記の半径自動拡大で部分的にカバー）。起点・目的地を別々に指定して経路上を検索する機能は未実装（将来対応）。
...
4. 起点・目的地を分けて指定できる複数エリア対応（新宿→渋谷のような移動を伴うコース）
5. Places Nearby Searchのページネーション対応（現状は1クエリあたり最大20件のみ取得）
